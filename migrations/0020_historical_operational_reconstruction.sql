-- Phase 1: Operational Reconstruction from JSON / Legacy Data
-- Target: agri-nile-flow-data-lake
-- Principle: Additive, Zero GL Impact, Deterministic Operational Traceability



-- 2. Generate Work Order Headers for Mechanization (SRV_MECH)
INSERT INTO work_orders (company_id, season_id, field_id, center_code, name, operation_type, planned_date, status, created_by, notes)
SELECT DISTINCT 
  st.company_id,
  st.season_id,
  f.id AS field_id,
  st.center_code,
  'ميكنة ' || COALESCE(f.name, 'حقل') || ' — ' || COALESCE(st.year, strftime('%Y', st.transaction_date)) || '/' || COALESCE(st.month, strftime('%m', st.transaction_date)) AS name,
  'SOIL_PREP' AS operation_type,
  MIN(st.transaction_date) AS planned_date,
  'costed' AS status,
  st.created_by_user_id,
  'توليد تلقائي: إعادة بناء تاريخي لعمليات الميكنة' AS notes
...
WHERE st.company_id = 1 
  ...
  AND st.work_order_id IS NULL
GROUP BY st.company_id, st.season_id, st.center_code, COALESCE(st.year, strftime('%Y', st.transaction_date)), COALESCE(st.month, strftime('%m', st.transaction_date));

-- 3. Generate Work Order Headers for Labor (SRV_LABOR)
INSERT INTO work_orders (company_id, season_id, field_id, center_code, name, operation_type, planned_date, status, created_by, notes)
SELECT DISTINCT 
  st.company_id,
  st.season_id,
  f.id AS field_id,
  st.center_code,
  'عمالة ' || COALESCE(f.name, 'حقل') || ' — ' || COALESCE(st.year, strftime('%Y', st.transaction_date)) || '/' || COALESCE(st.month, strftime('%m', st.transaction_date)) AS name,
  'MAINTENANCE' AS operation_type,
  MIN(st.transaction_date) AS planned_date,
  'costed' AS status,
  st.created_by_user_id,
  'توليد تلقائي: إعادة بناء تاريخي لعمليات العمالة' AS notes
FROM supplier_transactions st
JOIN fields f ON f.center_code = st.center_code AND f.company_id = st.company_id
WHERE st.company_id = 1 
