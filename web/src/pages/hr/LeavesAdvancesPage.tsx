...
import { TrendingDown, Plus, CheckCircle, XCircle, Clock, RefreshCw } from 'lucide-react'
...
export default function LeavesAdvancesPage() {
  ...
  const approvedAdv  = advances.filter(a => a.status === 'approved').length
  const totalAdvAmt  = advances.filter(a => a.status !== 'rejected').reduce((s, a) => s + a.amount, 0)
  const pendingLeave = leaves.filter(l => l.status === 'pending').length
  const totalLeaveDays = leaves.filter(l => l.status === 'approved').reduce((s, l) => s + l.days_count, 0)

  const kpis: KpiItem[] = tab === 'advances'
    ? [
        { id: 'total',    label: 'إجمالي طلبات السلف',    value: advances.length },
        { id: 'pending',  label: 'معلقة',                  value: pendingAdv,    variant: pendingAdv > 0 ? 'warning' : 'default' },
        { id: 'approved', label: 'معتمدة',                 value: approvedAdv,   variant: 'success' },
        { id: 'amount',   label: 'إجمالي المبالغ',         value: totalAdvAmt.toLocaleString() + ' ج.م' },
      ]
    : [
        { id: 'total',    label: 'إجمالي طلبات الإجازة',  value: leaves.length },
        { id: 'pending',  label: 'معلقة',                  value: pendingLeave,  variant: pendingLeave > 0 ? 'warning' : 'default' },
        { id: 'approved', label: 'أيام معتمدة',            value: totalLeaveDays, variant: 'success' },
        { id: 'types',    label: 'أنواع الإجازات',         value: leaveTypes.length },
      ]

  const actions: CommandAction[] = [
    {
      id: 'new',
      label: tab === 'advances' ? 'طلب سلفة' : 'طلب إجازة',
      icon: <Plus size={14} />,
      variant: 'primary',
      onClick: () => tab === 'advances' ? setShowAdvForm(true) : setShowLeaveForm(true),
    },
    { id: 'sep', isSeparator: true },
    { id: 'refresh', label: 'تحديث', icon: <RefreshCw size={14} />, variant: 'secondary',
      onClick: () => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc]">
      ...
      <div className="flex-1 overflow-auto p-6 space-y-5" dir="rtl">
          ...
          {['', 'pending', 'approved', 'rejected'].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`px-3 py-1 rounded-full text-xs border transition-all ${
                statusFilter === s ? 'bg-[#0F2D5C] text-white border-[#0F2D5C]' : 'text-slate-600 border-slate-200 hover:border-slate-400'
              }`}>
        ...
        {/* ADVANCES tab */}
        {tab === 'advances' && (
          <SectionCard title="طلبات السلف" icon={<TrendingDown size={16} />} badge={<span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{advances.length}</span>}>
            {advances.length === 0 ? (
              ...
              <div className="divide-y divide-slate-100">
                {advances.map(adv => {
                  ...
                  return (
                    <div key={adv.id} className="flex items-start gap-4 px-5 py-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-semibold text-slate-900">{adv.employee_name}</span>
                        ...
                        {adv.reason && <div className="text-xs text-slate-400 mt-0.5">{adv.reason}</div>}
                        <div className="text-xs text-slate-400 mt-1">{adv.request_date}</div>
                      </div>
                      {adv.status === 'pending' && (
        ...
        {/* LEAVES tab */}
        {tab === 'leaves' && (
          <SectionCard title="طلبات الإجازة" icon={<Clock size={16} />} badge={<span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{leaves.length}</span>}>
            {leaves.length === 0 ? (
              <div className="text-center py-12 text-slate-400">
              ...
              <div className="divide-y divide-slate-100">
                {leaves.map(lv => {
                  ...
                  return (
                    <div key={lv.id} className="flex items-start gap-4 px-5 py-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-semibold text-slate-900">{lv.employee_name}</span>
                          <span className={`text-xs px-2 py-0.5 rounded-full ${st.color}`}>{st.label}</span>
                          {lv.is_paid ? (
                            <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">مدفوعة</span>
                          ) : (
                            <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">بدون راتب</span>
                          )}
                        </div>
                        <div className="mt-1 text-sm text-slate-600">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setShowAdvForm(false)}
                className="flex-1 border rounded-lg py-2 text-sm text-slate-600 hover:bg-slate-50">إلغاء</button>
              <button
