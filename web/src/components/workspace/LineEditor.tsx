...
import { type ItemExtra } from './spreadsheet/SpreadsheetLookupCell'

// total_value replaces unit_price as the primary user input column
const FIELDS = ['item_code', 'pack_count', 'quantity', 'total_value']
...
interface Props {
  ...
  onUpdateLine:  (rowId: string, field: string, value: unknown, extra?: ItemExtra) => void
...
export default function LineEditor({ lines, movementType, warehouse, onAddLine, onRemoveLine, onUpdateLine, onApplyPaste }: Props) {
  ...
  const handleUpdateCell = (
    ...
    // item_code selection carries name + unit + packaging metadata.
    // The reducer handles all item-selection side-effects (packaging, reset) in one action.
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div
        ...
        style={{ gridTemplateColumns: '32px 2fr 70px 80px 90px 110px 80px 80px 28px', minWidth: '780px' }}
      >
        <span className="text-center">#</span>
        <span className="px-2">الصنف</span>
        <span className="text-center">عبوات</span>
        <span className="text-center">وحدة</span>
        <span className="text-center">كمية (وحدة)</span>
      ...
      <div className="flex-1 overflow-y-auto py-1 space-y-0.5 px-2" style={{ minWidth: '780px' }} onPaste={handleGridPaste}>
        {lines.map((line, idx) => (
          <LineRow
            key={line._key}
            line={line}
