  ...
  return rows
    .map(row => {
      ...
      for (const [field, colRef] of Object.entries(colMap)) {
        // colRef can be 'A', 'B', '__EMPTY', '__EMPTY_1', etc.
        let value = null
        
        if (typeof colRef === 'string' && colRef.length === 1 && colRef >= 'A' && colRef <= 'Z') {
          // It's a letter column reference (A-Z)
          const idx = colRef.charCodeAt(0) - 65
          value = row[idx] ?? null
        } else if (colRef.startsWith('__EMPTY')) {
          // It's an __EMPTY reference — parse the suffix
          if (colRef === '__EMPTY') {
            value = row[0] ?? null  // First column
          } else {
            const suffix = colRef.slice(7)  // Remove '__EMPTY' prefix
            const idx = parseInt(suffix, 10)
            if (!isNaN(idx)) {
              value = row[idx + 1] ?? null  // __EMPTY_1 → column 2 (0-indexed 1)
            }
          }
        }
