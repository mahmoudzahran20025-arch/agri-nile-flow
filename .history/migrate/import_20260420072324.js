...
async function importTreasury() {
  ...
  for (let i = 0; i < valid.length; i += 200) {
    ...
    const sql = batch.map(r => {
      ...
      const d = new Date(date)
      return `INSERT INTO cash_transactions (company_id, transaction_date, direction, document_number, recipient_name, narration, amount, debit, credit, running_balance, notes, year, month, created_by_user_id) VALUES (${COMPANY_ID}, ${esc(date)}, ${esc(r.direction)}, ${esc(r.document_number ? Number(r.document_number) : null)}, ${esc(r.recipient_name)}, ${esc(r.narration)}, ${amt}, ${debit}, ${credit}, ${runningBalance}, NULL, ${d.getFullYear()}, ${d.getMonth()+1}, 1);`
    }).filter(Boolean).join('\n')
...
async function importInventory() {
  ...
  for (let i = 0; i < valid.length; i += 100) {
    ...
    const sql = batch.map(r => {
      ...
      return `INSERT INTO inventory_movements (company_id, item_code, movement_date, warehouse, movement_type, quantity, unit_price, qty_in, qty_out, value_in, value_out, balance_qty, balance_value, document_number, notes, year, month, created_by_user_id) VALUES (${COMPANY_ID}, 0, ${esc(date)}, ${esc(wh)}, ${esc(mtype)}, ${qty}, ${unitPrice || 0}, ${qtyIn}, ${qtyOut}, ${valIn}, ${valOut}, ${balQty}, ${balVal}, NULL, ${esc(item)}, ${d.getFullYear()}, ${d.getMonth()+1}, 1);`
    }).filter(Boolean).join('\n')
