...
-- Enrich journal entry description so UI shows clear operational البيان (item/warehouse/source)
UPDATE journal_entries
SET description = (
  SELECT
    'Inventory #' || im.id || ' ' || COALESCE(im.movement_type, '-') ||
    ' | Item ' || COALESCE(CAST(im.item_code AS TEXT), '-') ||
    ' ' || COALESCE(it.name, '') ||
    ' | WH ' || COALESCE(im.warehouse, '-')
  FROM inventory_movements im
  LEFT JOIN items it
    ON it.company_id = im.company_id
   AND it.code = im.item_code
  WHERE im.company_id = journal_entries.company_id
    AND im.id = journal_entries.ref_id
)
WHERE company_id = 1
  AND ref_type = 'inventory_movement'
...
SET description = (
  SELECT
    'Supplier #' || st.id ||
    ' | Supplier ' || COALESCE(CAST(st.supplier_code AS TEXT), '-') ||
...
SET description = (
  SELECT
    'Cash #' || ct.id ||
    ' | Dir ' || COALESCE(ct.direction, '-') ||
    ' | Doc ' || COALESCE(CAST(ct.document_number AS TEXT), '-')
  FROM cash_transactions ct
  WHERE ct.company_id = journal_entries.company_id
    AND ct.id = journal_entries.ref_id
)
WHERE company_id = 1
  AND ref_type = 'cash_transaction'
  AND ref_id IS NOT NULL;

-- Keep line descriptions aligned with header description for readability in ledger UI
...
-- Propagate center_code from source records into posted journal lines
UPDATE journal_entry_lines
SET center_code = (
  SELECT im.center_code
  FROM journal_entries je
  JOIN inventory_movements im
    ON im.company_id = je.company_id
   AND im.id = je.ref_id
  WHERE je.id = journal_entry_lines.entry_id
...
SET center_code = (
  ...
  JOIN cash_transactions ct
    ON ct.company_id = je.company_id
   AND ct.id = je.ref_id
  WHERE je.id = journal_entry_lines.entry_id
...
-- Build business events for existing posted journal entries (traceability source event)
...
-- Create source documents bridge records
...
SELECT
  je.company_id,
  je.ref_type AS source_module,
  CAST(je.ref_id AS TEXT) AS source_id,
  je.ref_type AS document_type,
  (
    SELECT be.id
    FROM business_events be
    WHERE be.company_id = je.company_id
